...
import { pollSmartIdSession, findOrCreateEidUser } from '@/lib/eid';
...
export async function GET(_request: NextRequest) {
  try {
    ...
    if (eidSession.method !== 'smart-id') {
      return NextResponse.json({ error: 'Vale autentimismeetod' }, { status: 400 });
    }

    const result = await pollSmartIdSession(eidSession.sessionId);

    if (result.state === 'RUNNING') {
      return NextResponse.json({ state: 'RUNNING' });
    }

    // Non-RUNNING: clear eid_session and build a response
    ...
    console.error('Smart-ID poll error:', error);
